 ...
 * User: ~/.config/security-scanner/config.json (all platforms).
 * System: /etc/security-scanner/config.json (Unix), %ProgramData%\security-scanner\config.json (Windows).
 */
...
function getSystemConfigPath() {
  if (process.platform === 'win32') {
    const programData = process.env.ProgramData || path.join(process.env.SYSTEMROOT || 'C:\\Windows', 'System32', 'config', 'systemprofile');
    return path.join(programData, 'security-scanner', 'config.json');
  }
